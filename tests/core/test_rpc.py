...
"""
            ...
            rpc_registrations = []
            for handler_name in handlers.keys():
                rpc_registrations.append(
                    f"        self.bot.register_rpc_handler(self.{handler_name})"
                )

            # Generate cog class code
            cog_code = textwrap.dedent(
                f"""
...
async def test_rpc_handler_updates_on_reload(red, core_logic, test_cog_module, tmpdir):
    ...
    try:
        ...
        assert id(original_handler) != id(
            new_handler
        ), "Handler object identity should differ after reload"
...
async def test_rpc_reload_flow_matches_unload_load(red, core_logic, test_cog_module, tmpdir):
    ...
    try:
        ...
        assert (
            new_handler is not final_handler
        ), "Handler reference should update after manual load"
...
async def test_multiple_rpc_handlers_update_on_reload(red, core_logic, test_cog_module, tmpdir):
    ...
    cog_name = "multitest"
    handlers = {"handler_one": "one_v1", "handler_two": "two_v1", "handler_three": "three_v1"}
    test_cog_module.create_module(cog_name, handlers, tmpdir)
    ...
    try:
        ...
        handler_names = [
            f"{cog_name.upper()}__HANDLER_ONE",
        ...
        for i, (original, new) in enumerate(zip(original_handlers, new_handlers)):
            ...
            assert id(original) != id(
                new
            ), f"Handler {i+1} object identity should differ after reload"
...
async def test_rpc_reload_flow_via_rpc_interface(red, core_logic, test_cog_module, tmpdir):
    ...
    try:
        ...
        assert (
            original_handler is not new_handler
        ), "Handler reference should update after rpc_reload"
        assert id(original_handler) != id(
            new_handler
        ), "Handler object identity should differ after rpc_reload"
...
async def test_rpc_reload_via_websocket_endpoint_smoke_test(
    red, core_logic, test_cog_module, tmpdir
):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.ws_connect(f"ws://localhost:{server_port}/jsonrpc") as ws:
                ...
                await ws.send_json(
                    {"jsonrpc": "2.0", "method": handler_name, "params": [], "id": 1}
                )
            ...
            async with session.ws_connect(f"ws://localhost:{server_port}/jsonrpc") as ws:
                ...
                assert (
                    "error" not in result
                ), f"RPC reload failed: {result.get('error', 'Unknown error')}"
            ...
            async with session.ws_connect(f"ws://localhost:{server_port}/jsonrpc") as ws:
                ...
                await ws.send_json(
                    {"jsonrpc": "2.0", "method": handler_name, "params": [], "id": 3}
                )

                # Receive response
                result = await ws.receive_json()
                assert (
                    result["result"] == "http_v2"
                ), "Handler should execute new code after WebSocket RPC reload"
                assert "error" not in result
            ...
            async with session.ws_connect(f"ws://localhost:{server_port}/jsonrpc") as ws:
                for i in range(3):
                    ...
                    await ws.send_json(
                        {"jsonrpc": "2.0", "method": handler_name, "params": [], "id": 10 + i}
                    )
                    ...
                    assert (
                        result["result"] == "http_v2"
                    ), f"Handler should be consistent on call {i+1}"

            # Test 5: Verify error handling for non-existent methods
            async with session.ws_connect(f"ws://localhost:{server_port}/jsonrpc") as ws:
                # Send request for non-existent method
                await ws.send_json(
                    {"jsonrpc": "2.0", "method": "NONEXISTENT__METHOD", "params": [], "id": 4}
                )
