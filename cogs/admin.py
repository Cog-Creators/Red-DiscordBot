...
class Admin:
    ...
    async def addrole(self, ctx, rolename, user: discord.Member=None):
        ...
        if role is None:
            await self.bot.say('<:error:350172479936921611> That role cannot be found.')
            return

        if not channel.permissions_for(server.me).manage_roles:
            await self.bot.say('<:error:350172479936921611> I don\'t have manage_roles.')
            return

        await self.bot.add_roles(user, role)
        em = discord.Embed(description="<:success:350172481186955267> Added " + role.mention + " to " + user.name, color=role.color)
        await self.bot.say(embed=em)
        await self.bot.say('Added role {} to {}'.format(role.name, user.name))

    @commands.group(pass_context=True, no_pm=True, aliases=['adset'])
    async def adminset(self, ctx):
        """Manage Admin settings"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @adminset.command(pass_context=True, name="selfroles", aliases=['sr'])
    @checks.admin_or_permissions(manage_roles=True)
    ...
    async def removerole(self, ctx, rolename, user: discord.Member=None):
        ...
        if role in user.roles:
            try:
                ...
                await self.bot.say("<:remove:350172480498958337> Role successfully removed.")
            except discord.Forbidden:
                await self.bot.say("<:error:350172479936921611> I don't have permissions to manage roles!")
        else:
            await self.bot.say("<:error:350172479936921611> User does not have that role.")

    @commands.group(no_pm=True, pass_context=True, invoke_without_command=True, aliases=['sr'])
