...
class Economy:
    ...
    async def register(self, ctx):
        ...
        if user.id in self.bank:
            ...
            await self.bot.say(
                "{} Account opened. Current balance: {}".format(
                    user.mention, str(self.check_balance(server.id, user.id))))
        else:
            await self.bot.say(
    ...
    async def balance(self, ctx, user: discord.Member=None):
        ...
        if not user:
            user = ctx.message.author
            if self.account_check(server.id, user.id):
                await self.bot.say("{} Your balance is: {}".format(user.mention, str(self.check_balance(server.id, user.id))))
            else:
            ...
            if self.account_check(server.id, user.id):
                balance = self.check_balance(server.id, user.id)
    ...
    async def transfer(self, ctx, user: discord.Member, sum: int):
        ...
        if sum < 1:
            ...
            return
        if self.account_check(server.id, user.id):
            if self.enough_money(server.id, author.id, sum):
                self.withdraw_money(server.id, author.id, sum)
                self.add_money(server.id, user.id, sum)
    ...
    async def _set(self, ctx, user: discord.Member, sum: int):
        ...
        author = ctx.message.author
        server = ctx.message.server
        done = self.set_money(server.id, user.id, sum)
        if done:
    ...
    async def payday(self, ctx):
        ...
        id = author.id
        server = ctx.message.server
        if self.account_check(server.id, id):
            if server.id in self.payday_register and id in self.payday_register[server.id]:
                seconds = abs(
                    self.payday_register[server.id][id] - int(time.perf_counter()))
                if seconds >= self.settings["PAYDAY_TIME"]:
                    self.add_money(server.id, id, self.settings["PAYDAY_CREDITS"])
                    self.payday_register[server.id][id] = int(time.perf_counter())
    ...
    async def leaderboard(self, ctx, top: int=10):
        ...
        server = ctx.message.server
        if server.id not in self.bank:
            await self.bot.say("There is no one registered on this server... :/")
            return
        if top < 1:
            top = 10
        bank_sorted = sorted(
            self.bank[server.id].items(), key=lambda x: x[1]["balance"], reverse=True)
        ...
        for id in topten:
            ...
            highscore += (id[1]["name"] + " ").ljust(23 -
                                                     len(str(id[1]["balance"])))
    ...
    async def slot(self, ctx, bid: int):
        ...
        if sid not in self.slot_register:
            self.slot_register[sid] = {}
        if self.enough_money(sid, author.id, bid):
            if bid >= self.settings["SLOT_MIN"] and bid <= self.settings["SLOT_MAX"]:
                if author.id in self.slot_register[sid]:
                    if abs(self.slot_register[sid][author.id] - int(time.perf_counter())) >= self.settings["SLOT_TIME"]:
                        self.slot_register[sid][author.id] = int(
                            time.perf_counter())
                    ...
                    self.slot_register[sid][author.id] = int(time.perf_counter())
                    await self.slot_machine(ctx.message, bid)
            else:
    ...
    async def slot_machine(self, message, bid):
        ...
        if line[0] == ":two:" and line[1] == ":two:" and line[2] == ":six:":
            ...
            self.withdraw_money(message.server.id, message.author.id, bid)
            await self.bot.send_message(message.channel, "Credits left: {}".format(str(self.check_balance(message.server.id, message.author.id))))
            return True
        self.add_money(message.server.id, message.author.id, bid)
        await self.bot.send_message(message.channel, "Current credits: {}".format(str(self.check_balance(message.server.id, message.author.id))))
    ...
    async def economyset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            for k, v in self.settings.items():
                msg += str(k) + ": " + str(v) + "\n"
            msg += "\nType {}help economyset to see the list of commands.```".format(
                ctx.prefix)
    ...
    def check_balance(self, sid, id):
        if self.account_check(sid, id):
            return self.bank[sid][id]["balance"]
        else:
            return False

    def add_money(self, sid, id, amount):
        if self.account_check(sid, id):
            self.bank[sid][id]["balance"] = self.bank[sid][id]["balance"] + int(amount)
            fileIO("data/economy/bank.json", "save", self.bank)
        else:
            return False

    def withdraw_money(self, sid, id, amount):
        if self.account_check(sid, id):
            if self.bank[sid][id]["balance"] >= int(amount):
                self.bank[sid][id]["balance"] = self.bank[sid][id]["balance"] - int(amount)
                fileIO("data/economy/bank.json", "save", self.bank)
            else:
                return False
    ...
    def enough_money(self, sid, id, amount):
        if self.account_check(sid, id):
            if self.bank[sid][id]["balance"] >= int(amount):
                return True
            else:
    ...
    def set_money(self, sid, id, amount):
        if self.account_check(sid, id):
            self.bank[sid][id]["balance"] = amount
            fileIO("data/economy/bank.json", "save", self.bank)
            return True
        else:
            return False
...
def check_files():
    settings = {"PAYDAY_TIME": 300, "PAYDAY_CREDITS": 120,
        ...
        if current.keys() != settings.keys():
            for key in settings.keys():
                if key not in current.keys():
                    ...
                    print(
                        "Adding " + str(key) + " field to economy settings.json")
