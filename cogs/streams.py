...
class Streams:
    ...
    async def hitbox(self, stream: str):
        ...
        if online is True:
            await self.bot.say("*It looks like* **@" + stream + "** is streaming on **Hitbox**: http://www.hitbox.tv/{}/".format(stream))
    ...
    async def twitch(self, ctx, stream: str):
        ...
        if online is True:
            await self.bot.say("*It looks like* **@" + stream + "** is streaming on **Twitch**: http://www.twitch.tv/{}".format(stream))
        elif online is False:
            await self.bot.say("*Sorry but* **@" + stream + "** is **Offline**.")
        elif online == 404:
            await self.bot.say("Nope... That stream doesn't exist.")
    ...
    async def beam(self, stream: str):
        ...
        if online is True:
            await self.bot.say("*It looks like* **@" + stream + "** is streaming on **Beam**: https://beam.pro/{}".format(stream))
        elif online is False:
            await self.bot.say("*Sorry but* **@" + stream + "** is **Offline**.")
        elif online is None:
            await self.bot.say("Nope... That stream doesn't exist.")
                    ...
                    for channel in stream["CHANNELS"]:
                        ...
                        if channel_obj and can_speak:
                            await self.bot.send_message(
                                ...
                                "*I've been told to have you check out who just started streaming on* **Twitch**"
                                ": http://www.twitch.tv/{}".format(stream["NAME"]))
                else:
                ...
                if online is True and not stream["ALREADY_ONLINE"]:
                    ...
                    for channel in stream["CHANNELS"]:
                        ...
                        if channel_obj and can_speak:
                            await self.bot.send_message(
                                self.bot.get_channel(channel),
                                "*I've been told to have you check out who just started streaming on* **HitBox**"
                                ": http://www.hitbox.tv/{}".format(stream["NAME"]))
                else:
                    if stream["ALREADY_ONLINE"] and not online:
                        stream["ALREADY_ONLINE"] = False
                await asyncio.sleep(0.5)
                ...
                if online is True and not stream["ALREADY_ONLINE"]:
                    ...
                    for channel in stream["CHANNELS"]:
                        ...
                        if channel_obj and can_speak:
                            await self.bot.send_message(
                                self.bot.get_channel(channel),
                                "*I've been told to have you check out who just started streaming on* **Beam.pro**"
                                ": https://beam.pro/{}".format(stream["NAME"]))
                else:
                    if stream["ALREADY_ONLINE"] and not online:
                        stream["ALREADY_ONLINE"] = False
                await asyncio.sleep(0.5)
