...
class Shop:
    """Purchase server created items with Towels."""
    ...
    async def _buy_shop(self, ctx, quantity: int, *, itemname):
        """Purchase a shop item with Towels."""
        server = ctx.message.server
        user = ctx.message.author
        settings = self.check_server_settings(server)
        self.user_check(settings, user)
        ...
        if quantity < 1:
            ...
            if await self.shop_check(user, settings, quantity, itemname):
                if settings["Config"]["Pending Type"] == "Manual":
                    ...
                    await self.bot.say("```You have purchased {} {}(s) for {} Towels.\n"
                    ...
                    if not msgs:
                        ...
                        cost = self.discount_calc(settings, itemname)
                        self.shop_item_remove(settings, quantity, itemname)
                        await self.bot.whisper("You purchased {} {}(s) for {} Towels. "
    ...
    async def subtract_credits(self, user, number):
        ...
        if bank.account_exists(user):
            if bank.can_spend(user, number):
                ...
                await self.bot.say("You do not have enough Towels in your account.")
                return False
        else:
