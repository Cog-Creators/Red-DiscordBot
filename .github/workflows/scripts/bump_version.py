...
if int(os.environ.get("JUST_RETURN_VERSION", 0)):
    print(f"::set-output name=version::{bluebot.__version__}")
    sys.exit(0)
...
def repl(match: Match[str]) -> str:
    ...
    if new_stable_version == "auto":
        version_info = bluebot.VersionInfo.from_str(match.group("version"))
        version_info.dev_release = None
    else:
        version_info = bluebot.VersionInfo.from_str(new_stable_version)
...
with open("bluebot/__init__.py", encoding="utf-8") as fp:
...
with open("bluebot/__init__.py", "w", encoding="utf-8", newline="\n") as fp:
