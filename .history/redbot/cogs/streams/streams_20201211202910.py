    ...
    async def check_messages(self, ctx: commands.Context, streamer_name: str):
        ...
        for stream in self.streams:
            if stream.name.lower() == streamer_name.lower():
                msg = ""
                try:
                    nomention = "`{}`'s nomention message is set as: `{}`\n".format(streamer_name, stream.nomention_message)
                    # await ctx.send(_("`{}`'s nomention message is set as: `{}`".format(streamer_name, stream.nomention_message)))
                except AttributeError:
                    nomention = "`{}` does not have a nomention message.".format(streamer_name)
                    # await ctx.send(_("`{}` does not have a nomention message.".format(streamer_name)))
                msg += nomention

                try:
                    # await ctx.send(_("`{}`'s mention message is set as: `{}` to `{}`".format(streamer_name, stream.mention_message,
                    #                                                                          stream.who_to_mention)))
                    mention = "`{}`'s mention message is set as: `{}` to `{}`".format(streamer_name, stream.mention_message,
                                                                                      stream.who_to_mention)
                except AttributeError:
                    mention = "`{}` does not have a mention message.".format(streamer_name)
                    # await ctx.send(_("`{}` does not have a mention message.".format(streamer_name)))
                msg += mention
                
                await ctx.send(
                    embed=discord.Embed(
                    colour=await ctx.embed_colour(), description=msg
                )
        )
                return
        ...
        await ctx.send(
            embed=discord.Embed(
                ...
                description="Streamer `{}` is not registered and therefore does not have any custom messages.".format(streamer_name)
            )
        )
        return
