...
class Streams(commands.Cog):
    ...
    async def check_messages(self, ctx: commands.Context, streamer_name: str):
        ...
        await ctx.send(
            embed=discord.Embed(
                colour=await ctx.embed_colour(), description="Streamer `{}` is not registered and therefore does not have any custom messages.".format(streamer_name)
            )
        )
        await ctx.send(_("Streamer `{}` is not registered and therefore does not have any custom messages.".format(streamer_name)))
        return
