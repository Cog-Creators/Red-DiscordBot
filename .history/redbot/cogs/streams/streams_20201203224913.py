            ...
            if stream.name.lower() == name.lower():
                ...
                if mention == "mention":
                    if not hasattr(stream, "mention_message"):
                        ctx.send(_("{} does not have a mention message".format(stream.name)))
                        break
                    stream.__delattr__("mention_message")
                    print("mention message removed")
                elif mention == "nomention":
                    if not hasattr(stream, "nomention_message"):
                        ctx.send(_("{} does not have a nomention message.".format(stream.name)))
                        break
                    stream.__delattr__("nomention_message")
                    print("nomention message removed")
                elif mention == "both":
                    if not hasattr(stream, "nomention_message") or not hasattr(stream, "mention_message"):
                    stream.__delattr__("mention_message")
                    stream.__delattr__("nomention_message")
                    print("both removed")
                else:
