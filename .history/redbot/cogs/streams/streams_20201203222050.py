...
class Streams(commands.Cog):
    ...
    @commands.guild_only()
    async def custom_message(self, ctx: commands.Context, mention: str, name: str, msg: str):
        """Set custom stream alert message for already-registered streamer.
        
        Use the command `[p]streamset message streamer <mention|no-mention> <streamer name> <message>`
        ...
        for stream in self.streams:
            if stream.name.lower() == name.lower():
                not_found = False
                stream.__setattr__("message", msg)

        print(self.streams[0].message)
