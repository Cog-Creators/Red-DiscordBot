...
log = logging.getLogger("redbot.msgpack_driver")
...
class MsgpackDriver(BaseDriver):
    """
    Subclass of :py:class:`.BaseDriver`.

    .. py:attribute:: file_name

        The name of the file in which to store msgpack data.
    ...
    def __init__(
        ...
        *,
        ...
        file_name_override: str = "settings.pack",
    ):
    ...
    def _load_data(self):
        ...
        try:
            with self.data_path.open("rb") as fs:
                self.data = msgpack.load(fs, use_list=True)
        except FileNotFoundError:
            self.data = {}
            with self.data_path.open("wb") as fs:
                fs.write(msgpack.packb(self.data))
    ...
    def migrate_identifier(self, raw_identifier: int):
        ...
        for ident in poss_identifiers:
            if ident in self.data:
                ...
                del self.data[ident]
                _save_pack(self.data_path, self.data)
                break
    ...
    async def set(self, identifier_data: IdentifierData, value=None):
        partial = self.data
        full_identifiers = identifier_data.to_tuple()[1:]
        # This is both our deepcopy() and our way of making sure this value is
        # actually JSON serializable. msgpack supports more than JSON,
        # but JSON support is required for data conversion
    ...
    async def aiter_cogs(cls) -> AsyncIterator[Tuple[str, str]]:
        ...
        for _dir in data_manager.cog_data_path().iterdir():
            fpath = _dir / "settings.pack"
            if not fpath.exists():
                continue
            with fpath.open(mode="rb") as f:
                try:
                    data = msgpack.load(f, use_list=True)
                except Exception:
                    continue
    ...
    async def _save(self) -> None:
        ...
        await loop.run_in_executor(None, _save_pack, self.data_path, self.data)
