...
class Streams:
    ...
    async def all(self, ctx: commands.Context):
        ...
        if current_setting:
            ...
            await ctx.send(
                _("{} will no longer be mentioned when a stream or community is live").format("@\u200beveryone")
            )
        else:
            await self.db.guild(guild).mention_everyone.set(True)
            await ctx.send(
                _(
                    "When a stream or community "
                    "is live, {} will be mentioned."
                ).format("@\u200beveryone")
            )
    ...
    async def online(self, ctx: commands.Context):
        ...
        if current_setting:
            ...
            await ctx.send(
                _("{} will no longer be mentioned for an alert.").format("@\u200bhere")
            )
        else:
