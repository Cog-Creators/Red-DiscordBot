...
class Mutes(VoiceMutes, commands.Cog, metaclass=CompositeMetaClass):
    ...
    async def _handle_server_unmutes(self):
        ...
        for g_id in to_clear:
            await self.config.guild_from_id(g_id).clear()
    ...
    async def _handle_channel_unmutes(self):
        ...
        for c_id in to_clear:
            await self.config.channel_from_id(c_id).clear()
        ...
        if mute_role or sent_instructions:
            ...
            if can_react:
                msg += _(
                    ...
                    "Reacting with \N{CROSS MARK} will end the mute attempt."
                )
            else:
                msg += _(
    ...
    async def mute_user(
        ...
        if mute_role:
            ...
            tasks = []
            for channel in guild.channels:
                tasks.append(self.channel_mute_user(guild, channel, author, user, reason))
            task_result = await asyncio.gather(*tasks)
            for success, issue in task_result:
                if not success:
                    mute_success.append(f"{channel.mention} - {issue}")
                else:
                    chan_id = next(iter(issue))
                    perms_cache[str(chan_id)] = issue[chan_id]
    ...
    async def channel_mute_user(
        ...
        return True, {str(channel.id): old_overs}
