...
class CustomCommands:
    ...
    async def cc_add_random(self, ctx: commands.Context, command: str):
        ...
        try:
            ...
            await ctx.send(
                _("This command already exists. Use `{com}` to edit it.").format(
                    com="{}customcom edit".format(ctx.prefix)
                )
            )
    ...
    async def cc_edit(self, ctx, command: str, *, text=None):
        ...
        try:
            ...
            await ctx.send(
                _("That command doesn't exist. Use " "`{com}` to add it.").format(
                    com="{}customcom add".format(ctx.prefix)
                )
            )
        ...
        if not response:
            await ctx.send(
                _(
                    ...
                    " Use `{com}` to start adding some."
                ).format(com="{}customcom add".format(ctx.prefix))
            )
            return
