...
class AudioSetCommands(MixinMeta):
    """
    All audioset commands.
    """
    ...
    @_audioset.group(name="globaldb")
    @checks.is_owner()
    async def _audioset_globaldb(self, ctx: commands.Context):
        """Change global db settings."""

    @_audioset_globaldb.command(name="toggle")
    async def _audioset_globaldb_toggle(self, ctx: commands.Context):
        """Toggle the server settings. Default is on."""
    ...
    @_audioset_globaldb.command(name="contribute")
    async def _audioset_globaldb_contribute(self, ctx: commands.Context):
        ...
        await self.api_interface.post_to_global(ctx, db_entries)
    ...
    async def _audioset_localpath(self, ctx: commands.Context, *, local_path=None):
        ...
        if not pred.result:
            with contextlib.suppress(discord.HTTPException):
                await info.delete()
            return
        temp = LocalPath(local_path, forced=True)
    ...
    @checks.mod_or_permissions(administrator=True)
    async def _audioset_max_length(self, ctx: commands.Context, seconds: Union[int, str]):
    ...
    @checks.is_owner()
    async def _audioset_storage(self, ctx: commands.Context, *, level: int = None):
    ...
    @checks.is_owner()
    async def _audioset_cache_age(self, ctx: commands.Context, age: int):
