...
log = logging.getLogger("red.cogs.Audio.cog.commands.Controller")


class ControllerCommands(MixinMeta):
    """
    ...
    async def _pause(self, ctx: commands.Context):
        ...
        if not player.current:
            return await self._embed_msg(ctx, title=_("Nothing playing."))
        description = self.get_track_description(player.current)
    ...
    async def _bump(self, ctx: commands.Context, index: int):
        ...
        description = self.get_track_description(removed)
        await self._embed_msg(
