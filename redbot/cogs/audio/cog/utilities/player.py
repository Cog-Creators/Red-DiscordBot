...
class PlayerUtilities(MixinMeta):
    """
    All player utilities.
    """
    ...
    async def _enqueue_tracks(
        ...
        if type(query) is not list:
            if not await self.is_allowed(ctx.guild, f"{query}", query_obj=query):
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            description = self.get_track_description(single_track)
    ...
    async def _get_spotify_tracks(self, ctx: commands.Context, query: Query):
        ...
        if not api_data["spotify_client_id"] or not api_data["spotify_client_secret"]:
            return await self._embed_msg(
                ctx,
                title=_("Invalid Environment"),
                description=_(
                    ...
                    "\nSee `{prefix}audioset spotifyapi` "
                    "for instructions."
    ...
    async def _skip_action(self, ctx: commands.Context, skip_to_track: int = None):
        ...
        if not player.current or (not player.queue and not autoplay):
            ...
            embed = discord.Embed(
                title=_("Track Skipped"), description=self.get_track_description(player.current)
            )
    ...
    async def _players_check(self):
        try:
            ...
            get_single_title = self.get_track_description_unformatted(current)
