...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def _skip_action(
        ...
        if skip_to_track is not None and skip_to_track != 1:
            ...
            await self.send_embed_msg(ctx, embed=embed)
        self.bot.dispatch(
            "red_audio_skip_track", player.guild, player.current, ctx.author
        )
        await player.play()
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            async for track in AsyncIter(tracks):
                ...
                if not await self.is_query_allowed(
                    ...
                    continue
                elif guild_data["maxlength"] > 0 and not (
                    can_skip and maxlength_bypass
                ):
            ...
            title = (
                _("Playlist Enqueued") if not query.is_album else _("Album Enqueued")
            )
            embed = discord.Embed(
